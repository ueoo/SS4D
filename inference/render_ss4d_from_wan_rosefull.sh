#!/usr/bin/env bash
# Render SS4D from_wan inference results with GT data camera view (same pattern as AnimateAnyMesh infer_trellis_sample_rosefull).
set -euo pipefail
...
TEST_DATA_ROOT="${TEST_DATA_ROOT:-/scr/yuegao/TRELLIS_datasets/BlenderFlowers_Rosemore50kfullTest_merged}"
...
SMALL_SCENE_NAME="Rosemore_050016"
LARGE_SCENE_NAME="Rosemore_050047"
SMALL_FRAME="090"
LARGE_FRAME="175"
SMALL_CAMERA_VIEW_IDX="11"
LARGE_CAMERA_VIEW_IDX="6"
...
run_sample "Rosemore_050016_090_from_wan_forward_s000_t${T_END}" "${SMALL_SCENE_NAME}" "${SMALL_FRAME}" "${SMALL_CAMERA_VIEW_IDX}"
run_sample "Rosemore_050047_175_from_wan_reverse_s000_t${T_END}" "${LARGE_SCENE_NAME}" "${LARGE_FRAME}" "${LARGE_CAMERA_VIEW_IDX}"
